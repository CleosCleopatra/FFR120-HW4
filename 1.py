...
# # Initialize the strategies at random, 0 and Nr only.
# L = np.random.randint(2, size=(L_side, L_side)) * Nr
...
for rep in range(N_rep):
    ...
    L_init = L.copy()
    nstrategies = np.zeros([Nr + 1, T_max])
    C_t = np.zeros(T_max)
    for r in range(T_max):
        L = lattice_turn(L, Nr, T, R, P, S, mu)
        nstrategies[:,r] = count_strategies(L, Nr)
        C_t[r] = np.sum(np.arange(Nr + 1) * nstrategies[:, r]) / (L_side * L_side)
    C_all.append(C_t)
    
    im0 = axes[0, rep].imshow(L_init, cmap='viridis', vmin=0, vmax=Nr)
    fig.colorbar(im0, ax=axes[0, rep], label = 'Stratergy')
    axes[0, rep].set_title('Initial strategies')

    #FInal

    im1=axes[1, rep].imshow(L, cmap='viridis', vmin=0, vmax=Nr)
    fig.colorbar(im1, ax=axes[1, rep], label = 'Stratergy')
    axes[1, rep].set_title('Final strategies')
plt.tight_layout()
plt.show()
...
for re in range(N_rep):
    plt.plot(C_all[re], color=cell_color_list[re % len(cell_color_list)], label=f'rep {re+1}' )
    
plt.xlabel("t")
plt.ylabel("counts")
plt.title("Average strategies (normal)")
plt.legend()
plt.show()